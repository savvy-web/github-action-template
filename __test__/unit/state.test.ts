...
import { ActionEnvironment, ActionOutputs, ActionState } from "@effected/github-actions";
 ...
 * `save` appends to `GITHUB_STATE`, `get` reads `STATE_<key>` from the
 * environment, and the value crosses between them as TEXT. An in-memory
 * double that hands the encoded object straight back round-trips schemas
 * that JSON cannot — which is exactly the bug this suite exists to catch
 * (`Schema.Option`'s encoded form does not survive; see `src/state.ts`).
 */
...
describe("StartTimeState", () => {
	it.effect("survives the runner state round trip", () =>
		Effect.gen(function* () {
			const value = StartTimeState.make({ startedAt: 1_722_700_000_000 });
			const { restored, published } = yield* acrossPhases(STATE_KEYS.startTime, value, StartTimeState);
			assert.strictEqual(restored.startedAt, value.startedAt);
			// The published text is the contract with the runner: JSON, one line,
			// and no class internals that only decode back to themselves.
	...
	it.effect("getOptional reports none when the earlier phase saved nothing", () =>
		Effect.gen(function* () {
			const missing = yield* Effect.flatMap(ActionState, (state) =>
				state.getOptional(STATE_KEYS.startTime, StartTimeState),
